...
HRESULT
Library_sys_dev_spi_native_System_Device_Spi_SpiBusInfo::NativeMaxClockFrequency___I4(CLR_RT_StackFrame &stack)
{
    NANOCLR_HEADER();
...
HRESULT
Library_sys_dev_spi_native_System_Device_Spi_SpiBusInfo::NativeMinClockFrequency___I4(CLR_RT_StackFrame &stack)
{
    NANOCLR_HEADER();
...
// TODO doesn't take into account of full duplex or sequential ( assumes
// sequential at the moment )
bool System_Device_IsLongRunningOperation(
    uint32_t writeSize, uint32_t readSize, bool fullDuplex, bool bufferIs16bits,
    int32_t byteTime, uint32_t &estimatedDurationMiliseconds) {
  if (bufferIs16bits) {
  ...
  if (estimatedDurationMiliseconds >
      CLR_RT_Thread::c_TimeQuantum_Milliseconds) {
...
HRESULT Library_sys_dev_spi_native_System_Device_Spi_SpiDevice::NativeTransfer(CLR_RT_StackFrame &stack, bool data16Bits) {
  NANOCLR_HEADER();
  {
    ...
    if (stack.m_customState == 0) {
      ...
      if (data16Bits) {
        ...
        if (writeBuffer != NULL) {
          // grab the pointer to the array by getting the first element of the
          // array
          writeData = (unsigned char *)writeBuffer->GetFirstElementUInt16();

          // get the size of the buffer by reading the number of elements in the
          // HeapBlock array
        ...
        if (writeSpanByte != NULL) {
          // get buffer
          writeBuffer =
              writeSpanByte[SpanByte::FIELD___array].DereferenceArray();
          if (writeBuffer != NULL) {
            // Get the write offset, only the elements defined by the span must
            // be written, not the whole array
            writeOffset =
                writeSpanByte[SpanByte::FIELD___start].NumericByRef().s4;

            // use the span length as write size, only the elements defined by
            // the span must be written
            writeSize =
                writeSpanByte[SpanByte::FIELD___length].NumericByRef().s4;
        ...
        if (readSpanByte != NULL) {
          ...
          if (readBuffer != NULL) {
            // Get the read offset, only the elements defined by the span must
            // be read, not the whole array
            readOffset =
                readSpanByte[SpanByte::FIELD___start].NumericByRef().s4;

            // use the span length as read size, only the elements defined by
            // the span must be read
      ...
      isLongRunningOperation = System_Device_IsLongRunningOperation(
          writeSize, readSize, fullDuplex, data16Bits,
          nanoSPI_GetByteTime(deviceId),
          (uint32_t &)estimatedDurationMiliseconds);

      if (isLongRunningOperation) {
        // if this is a long running operation, set a timeout equal to the
        // estimated transaction duration in milliseconds this value has to be
        // in ticks to be properly loaded by SetupTimeoutFromTicks() below
        ...
        // if m_customState == 0 then push timeout on to eval stack[0] then move
        // to m_customState = 1 Return current timeout value
      ...
      // Start SPI transfer
      // We can ask for async transfer by setting callback but it depends if
      // underlying supports it return of CLR_E_BUSY means async started
      hr = nanoSPI_Write_Read(deviceId, rws, (uint8_t *)writeData,
    ...
    if (stack.m_customState == 2) {
      ...
      while (eventResult) {
        ...
        if (nanoSPI_Op_Status(deviceId) == SPI_OP_COMPLETE) {
          // SPI driver is ready meaning that the SPI transaction(s) is(are)
          // completed
          break;
        }

        // non-blocking wait allowing other threads to run while we wait for the
        // Spi transaction to complete
        NANOCLR_CHECK_HRESULT(g_CLR_RT_ExecutionEngine.WaitEvents(
            stack.m_owningThread, *timeout, Event_SpiMaster, eventResult));
...
Library_sys_dev_spi_native_System_Device_Spi_SpiDevice::NativeOpenDevice___I4(
  ...
  config = pThis[Library_sys_dev_spi_native_System_Device_Spi_SpiDevice::
                     FIELD___connectionSettings]
               .Dereference();
  ...
  spiConfig.Spi_Bus =
      config[SpiConnectionSettings::FIELD___busId].NumericByRef().s4 - 1;

  spiConfig.DeviceChipSelect =
      config[SpiConnectionSettings::FIELD___csLine].NumericByRef().s4;
  ...
  // load CS active state from config (which is always PinValue.Low or
  // PinValue.High
//  spiConfig.ChipSelectActive =
//      (bool)config[SpiConnectionSettings::FIELD___chipSelectLineActiveState]
//          .NumericByRef()
//          .s4;

  spiConfig.Spi_Mode =
      (SpiMode)config[SpiConnectionSettings::FIELD___spiMode].NumericByRef().s4;
  spiConfig.DataOrder16 =
      (DataBitOrder)config[SpiConnectionSettings::FIELD___dataFlow]
          .NumericByRef()
          .s4;
  spiConfig.Clock_RateHz =
      config[SpiConnectionSettings::FIELD___clockFrequency].NumericByRef().s4;
  spiConfig.BusConfiguration =
      (SpiBusConfiguration)
          config[SpiConnectionSettings::FIELD___busConfiguration]
              .NumericByRef()
              .s4;
