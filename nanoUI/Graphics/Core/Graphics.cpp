...
void WriteFormattedChar(unsigned char c)
{
    ...
    if (lcdCursor >= CursorEndOfDisplay)
    {
        ...
        ////        lcdCursor -= TextColumns;
        ////        CLR_UINT16 *ScreenBufferStart = (CLR_UINT16 *)g_DisplayDriver.Attributes.TransferBuffer;
        ////        CLR_UINT16 transferBufferSize = g_DisplayDriver.Attributes.TransferBufferSize;
        ////        CLR_INT32 rowMemorySize = InternalFont::Font_Height() * g_DisplayDriver.Attributes.Width * 2;
        ////        memmove(
        ////            ScreenBufferStart,
        ////            (ScreenBufferStart + rowMemorySize),
        ////            g_DisplayDriver.Attributes.TransferBufferSize - rowMemorySize);
        lcdCursor = 0;
    }
}
...
void lcd_printf(const char *format, ...)
{
    ...
    char buffer[256];
    int numberOfCharacters = vsnprintf(buffer, sizeof(buffer), format, args);
